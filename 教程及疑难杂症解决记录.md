...
- [教程及疑难杂症解决记录](#教程及疑难杂症解决记录)
- [Git](#git)
  - [SSH配置](#ssh配置)
- [Python相关](#python相关)
  - [文本读取相关](#文本读取相关)
    - [编码错误相关](#编码错误相关)
  - [**Ubuntu平台和WIndows平台编码不一致导致在的问题**](#ubuntu平台和windows平台编码不一致导致在的问题)
    - [在Windows平台生成了文本文件, 在Ubuntu平台使用时出现问题](#在windows平台生成了文本文件-在ubuntu平台使用时出现问题)
- [编译相关](#编译相关)
  - [工具](#工具)
    - [MinGW64下载及使用](#mingw64下载及使用)
  - [常用编译命令](#常用编译命令)
    - [文件夹相关操作](#文件夹相关操作)
  - [不同平台执行](#不同平台执行)
    - [C++判别不同平台](#c判别不同平台)
  - [跨平台编译](#跨平台编译)
    - [CMake](#cmake)
    - [sh命令](#sh命令)
    - [sh判断当前平台并执行程序](#sh判断当前平台并执行程序)
    - [运行`.sh`脚本](#运行sh脚本)
    - [Windows平台编译可执行程序](#windows平台编译可执行程序)
- [软件使用问题](#软件使用问题)
  - [VirtualBox](#virtualbox)
- [环境配置](#环境配置)
  - [Ubuntu配置CUDA环境](#ubuntu配置cuda环境)
- [C/C++编译运行及解决](#cc编译运行及解决)
      - [空指针（未初始化）](#空指针未初始化)
...
```CMD
:: CMD 中使用
ssh-add %USERPROFILE%\.ssh\super-acme
```
